...
class HydroReport:
    ...
    def _plot_annual_peaks(self, save_path: str = None) -> plt.Figure:
        ...
        ax.bar(df['water_year'], df['peak_flow_cfs'], 
        ...
        ax.annotate(f'Mean: {mean_peak:,.0f} cfs\nMax: {max_peak:,.0f} cfs (WY {max_year})',
    ...
    def generate_report_text(self) -> str:
        ...
        method_name = 'Expected Moments Algorithm (EMA)' if r.method == AnalysisMethod.EMA else 'Method of Moments (MOM)'
...
"""
        ...
        if self._gage.peak_data is not None:
            df = self._gage.peak_data
            report += f"| Period of Record | WY {df['water_year'].min()} - {df['water_year'].max()} |\n"
...
"""
        ...
        if self._gage.peak_data is not None:
            for _, row in self._gage.peak_data.iterrows():
                date_str = row['peak_date'].strftime('%Y-%m-%d') if pd.notna(row['peak_date']) else 'N/A'
                report += f"| {int(row['water_year'])} | {date_str} | {row['peak_flow_cfs']:,.0f} |\n"
