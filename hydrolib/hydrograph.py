...
class Hydrograph:
    ...
    MONTH_LABELS: ClassVar[List[str]] = ['Oct', 'Nov', 'Dec', 'Jan', 'Feb', 'Mar', 
    ...
    @classmethod
    def plot_daily_timeseries(cls, daily_data: pd.DataFrame, 
                               site_name: str = None,
                               site_no: str = None, 
                               save_path: str = None,
        ...
        ax.plot(daily_data.index, daily_data['flow_cfs'], 
        ...
        ax.annotate(f'Period of Record: {start_yr}-{end_yr}',
    ...
    @classmethod
    def plot_summary_hydrograph(cls, daily_data: pd.DataFrame, 
                                 site_name: str = None,
                                 site_no: str = None, 
                                 save_path: str = None,
                                 figsize: Tuple[int, int] = (10, 6),
                                 percentiles: List[int] = None) -> plt.Figure:
        ...
        stats_df = df.groupby('dowy')['flow_cfs'].agg(
            ['mean', 'min', 'max'] + 
            [lambda x, p=p: np.nanpercentile(x, p) for p in percentiles]
        )
        ...
        if len(percentiles) >= 4:
            ax.fill_between(stats_df.index, stats_df[f'p{percentiles[0]}'], 
                          stats_df[f'p{percentiles[-1]}'], 
                          alpha=0.3, color='blue', 
                          label=f'{percentiles[0]}-{percentiles[-1]}th percentile')
            ax.fill_between(stats_df.index, stats_df[f'p{percentiles[1]}'],
        ...
        ax.annotate(f'Period of Record: {start_yr}-{end_yr} ({n_years} years)',
