...
from .core import (
    AnalysisMethod, FlowInterval, EMAParameters, FrequencyResults,
    kfactor, kfactor_array, grubbs_beck_critical_value,
    log_pearson3_cdf, log_pearson3_pdf
)
...
class FloodFrequencyAnalysis(ABC):
    ...
    STANDARD_AEP: ClassVar[np.ndarray] = np.array([
        0.995, 0.99, 0.95, 0.90, 0.80, 0.50, 0.20, 0.10, 
        0.04, 0.02, 0.01, 0.005, 0.002
    ])
    
    def __init__(self, peak_flows: np.ndarray,
    ...
    def _compute_weighted_skew(self, station_skew: float) -> Optional[float]:
        ...
        n = self.n
        mse_station = (6 / n) * (1 + (6/n) * station_skew**2 + 
                                  (15/(n**2)) * station_skew**4)
    ...
    def compute_confidence_limits(self, aep: np.ndarray = None,
    ...
    def plot_frequency_curve(self, site_name: str = None, 
                              site_no: str = None,
                              save_path: str = None,
        ...
        ax.scatter(x_data, sorted_flows, c='blue', s=40, zorder=5,
                  label='Observed Annual Peaks', edgecolors='darkblue', linewidth=0.5)
        
        # Plot fitted curve
        aep_curve = np.array([0.999, 0.995, 0.99, 0.95, 0.90, 0.80, 0.50, 0.20,
                             0.10, 0.04, 0.02, 0.01, 0.005, 0.002, 0.001])
        ...
        ax.plot(x_curve, Q_curve, 'b-', linewidth=2, 
                label=f'LP3 Fitted Curve ({method_label})', zorder=4)
        
        # Confidence limits
        if show_confidence:
            cl = self.compute_confidence_limits(aep_curve)
            x_cl = [prob_to_x(p) for p in cl['aep']]
            ax.fill_between(x_cl, cl['lower_5pct'], cl['upper_5pct'],
        ...
        if self._results.low_outlier_threshold > 0 and self._results.n_low_outliers > 0:
            ...
            for lo in low_outliers:
                ...
                if len(idx) > 0:
                    ax.scatter([x_data[i] for i in idx], [lo]*len(idx), 
                              c='red', s=60, marker='x', zorder=6)
            ax.axhline(threshold, color='red', linestyle=':',
                      alpha=0.7, label=f'Low Outlier Threshold ({self._results.n_low_outliers})')
        ...
        ax.set_xticklabels([f'{p*100:.1f}%' if p >= 0.01 else f'{p*100:.2f}%' 
                          for p in prob_ticks], fontsize=9, rotation=45, ha='right')
        ...
        ax2.set_xticks(rp_x)
        ax2.set_xticklabels([str(int(rp)) if rp >= 1 else f'{rp:.2f}' for rp in rp_ticks], fontsize=9)
...
class MethodOfMoments(FloodFrequencyAnalysis):
    ...
    def run_analysis(self) -> FrequencyResults:
        ...
        skew_station = (n * np.sum((log_flows - mean_log)**3) /
                       ((n-1) * (n-2) * std_log**3))
...
class ExpectedMomentsAlgorithm(FloodFrequencyAnalysis):
    ...
    def _build_flow_intervals(self, low_threshold: float = 0.0) -> List[FlowInterval]:
        ...
        for flow, year in zip(self._peak_flows, self._water_years):
            ...
            if flow < low_threshold:
                intervals.append(FlowInterval.from_censored(
                    lower=0, upper=low_threshold, year=year,
                    perception_threshold=0.0
                ))
            else:
        ...
        for year, flow in self._historical_peaks:
            ...
            intervals.append(FlowInterval.from_historical(
                flow, year, perception_threshold=threshold
            ))
        ...
        if self._ema_params.historical_start and self._ema_params.historical_threshold:
            ...
            for year in range(self._ema_params.historical_start, 
                            self._ema_params.historical_end + 1):
    ...
    def _compute_ema_moments(self, mean_log: float, std_log: float, 
    ...
    def _ema_iteration(self, mean_log: float, std_log: float, 
                        skew: float) -> Tuple[float, float, float]:
    ...
    def run_analysis(self) -> FrequencyResults:
        ...
        for iteration in range(self._ema_params.max_iterations):
            ...
            if (abs(new_mean - mean_log) < self._ema_params.tolerance and
                abs(new_std - std_log) < self._ema_params.tolerance and
                abs(new_skew - skew_station) < self._ema_params.tolerance):
                converged = True
                mean_log = new_mean
                std_log = new_std
