...
Tests: **494 passed, 1 deselected, 6 xfailed** in ~23 s with the Fortran extension and
Streamlit both installed (was ~75 s for far fewer tests; see the MGBT memoization below).
CI green on main. One xfail was retired by the bias-correction fix; the six that remain are
all the skew-weighting and CI-shape defects in P3.
...
- [x] **`n` in the LP3 parameters table.** First column, rendered `n = 117` to match the
      annotation the frequency curve already carries (`bulletin17c.py:491`). The count is
      `n_peaks`, carried out through `run_ffa`'s `parameters` dict — **not** `n_systematic`,
      which is computed as "systematic and not censored" (`bulletin17c.py:1331`) and so falls
      by one for every PILF MGBT finds. Reporting that as record length would double-report
      the PILF column beside it and make the record appear to change length when the
      threshold moves. Pinned by a test on Cains Coulee, where the two differ by 11.

      The raw dict also feeds `app/ffa_export.py`, so `n_peaks` now appears in the exported
      `lp3_parameters.csv` as well.

- [x] **`FFA Plot Options` label**, below the skew options. Named with the `FFA` prefix
      deliberately: `st.sidebar.header("Plot Options")` at `streamlit_app.py:72` already
      governs the hydrograph and flow-duration plots, and two controls sharing a name in one
      sidebar is worse than either name alone, divider or no divider.

- [x] **Skew-option checkboxes moved up** to sit directly under `Regional Skew SE`, so the
      three skew choices follow the two inputs that drive them.

- [x] **`Flood Frequency Analysis` heading and its divider moved above the
      `Peak Flow Years (for FFA)` selector**, which is an FFA control — it re-runs the fit —
      and now reads as one. This needed a second `st.sidebar.container()`, because a
      container reserves its sidebar position where it is *declared*, not where it is filled:
      the daily-flow range writes into the container declared above the divider and the
      peak-year selector into the one declared below the heading. `Update Plots` moved with
      the peak-year block, so it still sits below every input it applies to.

      Sidebar order is now: Date Range Filters → Daily Flow Range → *divider* →
      **Flood Frequency Analysis** → Peak Flow Years (for FFA) → Update Plots → Enable FFA →
      Regional Skew → Regional Skew SE → **Skew Options** → **FFA Plot Options** →
      **Low Outliers (PILFs)**.

      Verified by `make smoke`, which imports `app/streamlit_app.py` and so executes the
      whole sidebar body in Streamlit's bare mode.
